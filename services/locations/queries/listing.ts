...
const listing = async (context: QueryFunctionContext) => {
	...
	let url: string = Endpoints.LOCATIONS;
	...
	try {
		...
		throw Error(error.message, { cause: 'Error fetching locations' });
	}
};

export const useLocationsListing = (options: QueryOptions) => {
	const query = useQuery<RequestData<Location>, RequestError>({
		queryKey: [Endpoints.LOCATIONS, options],
		queryFn: listing
	});
